...
fun fireKaczkosznikov(ctx: Ctx, pos: Vector2, direction: Vector2) {
    ctx.engine.entity {
        body(ctx.world.body(BodyDef.BodyType.DynamicBody) {
            ...
            val vec = Vector2(direction.nor())
            position.set(pos.add(vec))
            angle = direction.angleRad()
            circle(ctx.params.kaczkosznikovSize / 2) {
                filter {
        ...
        script(KaczkocznikovProjectileScript(ctx.params.kaczkosznikovDamage, projectileAnimation, ctx))
    }
}

class KaczkocznikovProjectileScript(val damage: Float,
